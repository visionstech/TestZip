@extends('layouts.app')
@section('pageTitle', 'ASSESSMENT REIVEW') 
@section('css')
...
<div class="outer-search-list">
   ...
   <div class="search-list">
      <div class="search-heading">
         ...
         <p>Your current tax year {{ ($data['search_details']->appeal_year != "")?$data['search_details']->appeal_year: date('Y') }} assessment is <span>
         ${{ number_format($data['search_details']->total_assessment_value) }}</span>. Your house recently sold on {{ ($data['search_details']->sale_date != "")?date('m/d/Y', strtotime($data['search_details']->sale_date)): date('m/d/Y') }} for ${{ number_format($data['search_details']->sale_price) }}.  Based on the recent sale of your property and recent sales of other similar properties in your neighborhood, your property appears to be over assessed.  
        <br><br>
        The potential one year tax savings that could result from a successful appeal is ${{ number_format($data['search_details']->real_tax_amount) }}.  With this lower base value for tax year {{ ($data['search_details']->appeal_year != "")?$data['search_details']->appeal_year: date('Y') }} the potential savings for future years is even greater.</p>
